...
bool SendOnline() {
    ...
    if (!pub((roomsTopic + "/pir_timeout").c_str(), 0, true, toStr(pirTimeout).c_str())) return false;
    if (!pub((roomsTopic + "/radar_timeout").c_str(), 0, true, toStr(radarTimeout).c_str())) return false;
    online = true;
    return true;
}
}  // namespace Motion