...
const std::string LED::getId() {
    return Sprintf("led_%d", index);
}

const std::string LED::getStateFilename() {
    return Sprintf("/led_%d_state", index);
}

const std::string LED::getStateString() {
    ...
    sprintf(stateStr, "%02X%02X%02X%02X%02X",
            ...
            color.blue,
            color.white);
    return std::string(stateStr);
}

void LED::setStateString(const std::string& encoded) {
    if (encoded.length() == 10) {
        // Parse hex values - each value is 2 hex digits
        uint8_t const brightness = strtol(encoded.substr(0, 2).c_str(), NULL, 16);
        uint8_t const r = strtol(encoded.substr(2, 2).c_str(), NULL, 16);
        uint8_t const g = strtol(encoded.substr(4, 2).c_str(), NULL, 16);
        uint8_t const b = strtol(encoded.substr(6, 2).c_str(), NULL, 16);
        uint8_t const w = strtol(encoded.substr(8, 2).c_str(), NULL, 16);
