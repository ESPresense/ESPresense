...
void Setup() {
    std::vector<PinMode> pinTypes = {INPUT_PULLUP, INPUT_PULLUP, INPUT_PULLDOWN, INPUT_PULLDOWN, INPUT, INPUT};
 ...
 * Settings, storing values into the corresponding module variables:
...
bool SendOnline() {
    ...
    if (!pub((roomsTopic + "/switch_1_timeout").c_str(), 0, true, toStr(switch_1Timeout).c_str())) return false;
    if (!pub((roomsTopic + "/switch_2_timeout").c_str(), 0, true, toStr(switch_2Timeout).c_str())) return false;
    online = true;
    return true;
}
}  // namespace Switch