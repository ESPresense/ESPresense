 ...
 * from Settings and stores them in the corresponding globals:
...
bool SendOnline() {
    ...
    if (!pub((roomsTopic + "/button_2_timeout").c_str(), 0, true, toStr(button_2Timeout).c_str())) return false;
    online = true;
    return true;
}
}  // namespace Button