...
// Vendored from https://github.com/m5stack/M5Unit-ENV/blob/master/src/I2C_Class.cpp
// Modified: removed Wire.end() call in begin() to avoid disrupting other sensors on the bus
...
void I2C_Class::begin(TwoWire *wire, uint8_t sda, uint8_t scl, long freq) {
    ...
    _scl  = scl;
    _freq = freq;
    // Note: intentionally not calling _wire->end() here to avoid
    // disrupting other sensors already initialized on this bus
