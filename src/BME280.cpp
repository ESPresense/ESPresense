...
namespace BME280
{
    ...
    /**
     * @brief Initializes the BME280/BME68x sensor according to configured I2C address and bus.
     *
     * Determines the configured I2C address, then attempts to initialize a BME68x sensor first.
     * If BME68x is not found, falls back to BME280. Logs an error if neither sensor is detected.
     * If initialization succeeds, marks the sensor as initialized.
     */
    void Setup()
    {
    ...
    /**
     * @brief Logs the BME280/BME68x sensor I2C address, bus, and model to the system logger when available.
     *
    ...
    /**
     * @brief Periodically samples the BME280/BME68x sensor and publishes measurements to MQTT.
     *
     ...
     * topics "/bme280_temperature", "/bme280_humidity", and "/bme280_pressure". For BME68x sensors,
     * also publishes gas resistance to "/bme68x_gas". Updates the internal timestamp for the last
     * sample after successful read.
     */
    void Loop()
    {
        if (!I2C_Bus_1_Started && !I2C_Bus_2_Started) return;
