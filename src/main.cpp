...
void loop() {
    ...
    if (millis() - lastSlowLoop > 5000) {
        ...
        // the reporter's S3 leak drains (~8KB/6h; can't spill to the N16R8's PSRAM). The BLE flood
        // alone does NOT drain it (verified flat), so the accelerator is browser HTTP/ws traffic:
        // flash this, open the web UI, and watch dmaFree slide while fingerprints stay flat. (#2309)
