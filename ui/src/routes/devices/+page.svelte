<script lang="ts">
    ...
    let modalTitle = $derived(($events as Events)?.room ? $_('devices.enroll_device') + ` - ${($events as Events).room}` : $_('devices.enroll_device'));
    ...
    async function onDeleteConfig() {
        ...
        if (!confirm($_('devices.are_you_sure_delete_device') + ` "${editingConfig.name || editingConfig.alias || editingConfig.id}"?`)) {
            return;
        }
...
<div class="bg-gray-100 dark:bg-gray-800 rounded-lg shadow">
    <div class="p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white">{$_('devices.devices')}</h2>
            <button
        ...
        {:else}
            <div class="flex items-center justify-center min-h-[50vh]">
                <div class="text-center">
                    <h1 class="text-xl font-medium text-gray-700 dark:text-gray-300">{$_('devices.loading_configured_devices')}</h1>
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{$_('devices.please_wait_while_fetch')}</p>
                </div>
            </div>
        {/if}
    </div>
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full">
                <div class="flex justify-between items-start p-4 border-b dark:border-gray-700">
                    ...
                    <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-2 ml-auto inline-flex items-center dark:hover:bg-gray-700 dark:hover:text-white" onclick={onClose}>
                        ...
                        <span class="sr-only">{$_('devices.close')}</span>
                    </button>
                </div>
                    ...
                    {#if !($events as Events)?.state?.enrolling}
                        {#if ($events as Events)?.state?.enrolledId}
                            <p class="text-sm leading-5 text-gray-600 dark:text-gray-400">
                                {$_('devices.the_device_has_enrolled')}
                        ...
                        {:else}
                            <div class="space-y-4">
                                ...
                                >
                                    <option value="">{$_('devices.select_device_type')}</option>
                                ...
                                <input
                                    ...
                                    type="text"
                                    bind:value={name}
                                    placeholder={$_('devices.enter_name')}
                                />
                                <input
                                    class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                    type="text"
                                    bind:value={id}
                                    placeholder={name ? (deviceType ? `${deviceType}:${generateKebabCaseId(name, deviceType)}` : generateKebabCaseId(name)) : $_('devices.enter_custom_id')}
                                />
                                <p class="text-sm text-gray-600 dark:text-gray-400">{$_('devices.leave_empty_to_use_auto_id')}</p>
                            </div>
                        {/if}
                    {:else}
                        <p class="text-sm leading-5 text-gray-600 dark:text-gray-400">{$_('devices.to_begin_navigate_bt_settings')}</p>
                        <p class="mt-4 text-sm leading-5 text-gray-600 dark:text-gray-400">{$_('devices.start_pairing')} {formatRemainingTime(($events as Events).state?.remain ?? 0)}</p>
                    {/if}
                </div>
...
{#if showEditModal && editingConfig}
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity">
        <div class="flex items-center justify-center w-full h-full p-4">
            <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-lg w-full">
                <div class="flex justify-between items-start p-4 border-b dark:border-gray-700">
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white">{$_('devices.edit_device')}</h3>
                ...
                <div class="p-6 space-y-6">
                    <div class="space-y-4">
                        <div>
                            <label for="edit-device-id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{$_('devices.id')}</label>
                            <input
                                id="edit-device-id"
                                class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                type="text"
                        ...
                        <div>
                            <label for="edit-device-alias" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{$_('devices.alias')}</label>
                            <input
                        ...
                        <div>
                            <label for="edit-device-name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{$_('devices.name')}</label>
                            <input
                                id="edit-device-name"
                                class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                type="text"
                        ...
                        <div>
                            <label for="edit-device-rssi" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{$_('devices.rssi_at_1m')}</label>
                            <input
                                id="edit-device-rssi"
                                class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                type="number"
