<script lang="ts">
    ...
    let findPhase = $state<"close" | "far">("close");
    let signalHistory = $state(new Map<string, number[]>());
    let candidateDevices = $state(new Set<string>());
    ...
    function updateCandidates(rows: Device[]) {
        if (!findMode) {
    ...
    function isCandidateDevice(device: Device): boolean {
        ...
        if (phaseHistory.length < 3) return true;
    ...
    $effect(() => {
        ...
        if (findMode && currentDevices) {
            updateCandidates(currentDevices);
        }
    });
        ...
        <div class="p-6">
            ...
            {#if findMode}
                <div class="mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md">
                    <div class="text-sm font-medium text-gray-900 dark:text-gray-100">Find Mode Active</div>
            ...
            <div class="mt-3">
                ...
                <form onsubmit={onEnroll} class="space-y-4">
                    ...
                    {#if selectedDevice}
                        <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded-md">
                            ...
                            <div class="text-xs text-gray-600 dark:text-gray-400 space-y-1">
                                ...
                                {#if selectedDevice.rssi != null}<div>RSSI: {selectedDevice.rssi} dBm</div>{/if}
                    ...
                    <div class="flex justify-end space-x-3 pt-4">
                        <button
                            type="button"
                            onclick={() => { showEnrollModal = false; resetEnrollForm(); }}
